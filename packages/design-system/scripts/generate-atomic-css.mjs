...
// tokens.component;` statement is what actually positions `base` correctly
// in the global cascade-layer order: `tokens.component` was already named
// (and placed last, highest-priority) by tokens.css's own bare @layer
// statement, which loads first in every real consumer (tokens/css then
// design-system/css). Re-stating both names together here, in this order,
// inserts the new `base` layer immediately before tokens.component's
// already-established position — the standard CSS mechanism for a
// later-loaded stylesheet to slot a new layer into a specific relative spot
// without having to redeclare the full 5-layer tokens.* order itself.
 ...
 * hand-written src/base.css prepended verbatim (see decisions/decision-base-reset-stylesheet.md).
 * ${rules.length} base rules, ${Object.keys(breakpoints).length} responsive breakpoint tiers.
 */

@layer base, tokens.component;
